...
BEGIN  
 ...
 set @OpeningBalLT  =  CASE
  ...
  --------------------------------------BuyNotFound--------------------------------------------------------------------------
   ...
   --select @BuyNotFound=Sum(Profit) from Tax_Profit_Buynotfound_TDS where Clientid=@Client and Trandate=@TransSaleDate
   --if @BuyNotFound>0
   --begin
   ...
   WITH RankedPurchases AS (
	...
	--select * from SalesWithPurchase
	-- Assuming the temporary table #temp already exists with the appropriate columns
	INSERT INTO #temp (salesid, Selldate, sales_amount,sale_qty, Security, purchase_date, purchase_amount, purchase_qty,profit,POH)
	SELECT
	  salesid,
	  Trandate,
	  ...
	  corresponding_sales_amount-corresponding_purchase_amount,
   ...
   if exists(SELECT t.Security,sum(t.purchase_qty),sum(b.Qty)FROM #temp t INNER JOIN tax_Profit_Buynotfound_TDS b ON b.Security = t.Security AND b.TransId = t.salesid and b.Trandate=@TransSaleDate GROUP BY t.Security HAVING SUM(b.Qty) <> SUM(t.purchase_qty))
	...
	SELECT
	  ...
	  NULL AS purchase_date, 0 AS purchase_amount,0 AS purchase_qty,(SUM(b.Qty) - SUM(t.purchase_qty))*(b.SellValue/b.Qty),0
	FROM #temp t
