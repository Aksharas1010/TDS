...
create Procedure SpGetDailyTransactionDetails 
@frmdate date null,  
@todate date null,  
@clientid int null,  
@FiscYear varchar(12)     
...
WHERE tb.Trandate  between @frmdate and @todate AND tb.clientid = @clientid
GROUP BY
    c.NAME,
    c.TRADECODE,
    RTRIM(curlocation) + RTRIM(tradecode),
...
INNER JOIN Client c ON c.CLIENTID = tb.Clientid
LEFT JOIN Sauda s ON s.CLIENTID = tb.Clientid AND tb.Trandate= s.TRANDATE AND tb.Security = s.SECURITY
WHERE tb.Trandate  between @frmdate and @todate 
GROUP BY
    c.NAME,
    c.TRADECODE,
    RTRIM(curlocation) + RTRIM(tradecode),
